...
'''Diverse Communities: Almudena Chapa'''
...
def relocation_policy3_chapa(segregation_intmap, happiness_map, k, p, q):
    
    unhappies, empties = unhappy_and_empty_info_extraction_for_policies(segregation_intmap, happiness_map, k, p)
    
    # shuffle(empties)
    # print ('Antes', empties[0:5])
    # for each in empties:
    #     diversity=abs(each[1]-each[2])
    #     each.append(diversity)
    ...
    for unhappy in unhappies:
        ...
        if not found_happy_spot and found_less_unhappy_spot:
            ...
            new_empty_spot = unhappy
            new_empty_spot.pop(3)
            new_empty_spot.append(abs(new_empty_spot[1]-new_empty_spot[2]))
